...
async def signin(request: SigninRequest):
    """Sign in user - Supabase is the primary source of truth for existence"""
    init_db()
    conn = get_db()
    c = conn.cursor()
    
    try:
        # 1. MANDATORY: Check Supabase FIRST for account existence
        # If the user is deleted from Supabase, they MUST NOT be allowed to login
        try:
            sb_response = supabase.table("users").select("*").eq("email", request.email).execute()
            if not sb_response.data or len(sb_response.data) == 0:
                # Account deleted from Supabase -> Sync local DB and reject login
                c.execute("DELETE FROM users WHERE email = ?", (request.email,))
                c.execute("DELETE FROM signup_requests WHERE email = ?", (request.email,))
                conn.commit()
                print(f"🛑 Login rejected: {request.email} not found in Supabase. Local data cleaned up.")
                raise HTTPException(status_code=401, detail="⚠️ البريد الإلكتروني غير موجود. يرجى إنشاء حساب جديد")
            ...
            # If Supabase check fails due to error, we still reject login for security
            print(f"❌ Critical: Supabase check failed: {se}")
            raise HTTPException(status_code=503, detail="⚠️ فشل التحقق من الحساب. يرجى المحاولة لاحقاً")

        # 2. Check local DB for password hash
        c.execute("SELECT id, email, full_name, password_hash, organization, role, status FROM users WHERE email = ?", 
                 (request.email,))
        user_data = c.fetchone()
        
        if not user_data:
            # If in Supabase but not local DB, they need to signup/re-sync
            print(f"⚠️ User {request.email} exists in Supabase but missing locally.")
            raise HTTPException(status_code=401, detail="⚠️ بيانات تسجيل الدخول غير متوفرة محلياً. يرجى إعادة التسجيل")

        user_id, email, full_name, password_hash, organization, role, local_status = user_data
        
        # 3. Sync local data with Supabase data (Status, Role, Org, Name)
        sb_user = sb_response.data[0]
        sb_status = sb_user.get("status", "pending")
        sb_role = sb_user.get("role", "security_man")
        sb_org = sb_user.get("organization", "")
        sb_full_name = sb_user.get("full_name", "")

        if (local_status != sb_status or role != sb_role or 
            organization != sb_org or full_name != sb_full_name):
            c.execute("""
                UPDATE users 
                SET status = ?, role = ?, organization = ?, full_name = ? 
                WHERE email = ?
            """, (sb_status, sb_role, sb_org, sb_full_name, email))
            conn.commit()
            local_status, role, organization, full_name = sb_status, sb_role, sb_org, sb_full_name
        ...
        # Sync last login time to Supabase
        try:
